...
export function AchievementBadgesShowcase({
  ...
  if (earned.length === 0) {
    if (friendCompact) {
      return (
        <p className={`text-center text-xs ${isDarkMode ? "text-slate-500" : "text-slate-400"}`}>No badges yet</p>
      );
    }
    return (
      <div
  ...
  if (friendCompact) {
    return (
      <ul className="grid grid-cols-3 gap-3 sm:grid-cols-4 sm:gap-4" aria-label="Their achievement badges">
        {badgeList}
      </ul>
    );
  }
