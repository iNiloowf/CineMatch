...
export default function SharedWatchlistPage() {
  const { sharedMovieGroups, toggleWatched, toggleSharedMovie } = useAppState();
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-500">
                      {group.movies.length > 0
                        ? `${group.movies.filter((movie) => movie.shared).length} shared`
                        : "No shared picks yet"}
                    </p>
                  </div>
                </div>
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-500">
                    {detailsMovie.movie.year} • {detailsMovie.movie.runtime}
                  </p>
                </div>
                <span className="rounded-full bg-violet-100 px-3 py-1 text-xs font-semibold text-violet-700">
